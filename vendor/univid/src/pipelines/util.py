...
class BasePipeline(torch.nn.Module):
    ...
    def __init__(
        self,
        device: Optional[str] = "cuda", torch_dtype=torch.float16,
    ...
    def preprocess_image(self, image, torch_dtype=None, device=None, pattern="B C H W", min_value=-1, max_value=1):
        ...
        image = image * ((max_value - min_value) / 255) + min_value # [0, 255] -> [min_value, max_value], 一般是[-1, 1]
...
class ModelConfig:
    ...
    origin_file_pattern: Union[str, list[str]] = None  # type: ignore
...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        **kwargs 
        ...
        video_dict = {}
        iteration_num = inputs_shared["latents"].shape[1] // 16 # B C T H W
        for i in range(iteration_num):
            video = self.vae.decode(inputs_shared["latents"][:, i*16:(i+1)*16, ...], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
            video = self.vae_output_to_video(video) # tensor 2 Image
            video_dict[f"video_{i+1}"] = video
...
class PipelineUnit:
    ...
    def __init__(
        ...
        input_params_posi: Optional[dict[str, str]] = None,
...
class PipelineUnitRunner:
    ...
    def __call__(self, unit: PipelineUnit, pipe: WanVideoPipeline, inputs_shared: dict, inputs_posi: dict, inputs_nega: dict) -> tuple[dict, dict, dict]:
        if unit.take_over:
...
class WanVideoUnit_NoiseInitializer(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
        length = (num_frames - 1) // 4 + 1 # for vace_reference_image, it needs extra noise frame
        if vace_reference_image is not None:
            length += 1
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image, inference_image):
...
class WanVideoUnit_ImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
        ...
        if end_image is not None:
            ...
            vae_input = torch.concat([image.transpose(0,1), torch.zeros(3, num_frames-2, height, width).to(image.device), end_image.transpose(0,1)],dim=1) # 首尾帧
            if pipe.dit.has_image_pos_emb:
                clip_context = torch.concat([clip_context, pipe.image_encoder.encode_image([end_image])], dim=1)
            msk[:, -1:] = 1
        else:
            vae_input = torch.concat([image.transpose(0, 1), torch.zeros(3, num_frames-1, height, width).to(image.device)], dim=1) # i2v的input image
        ...
        y = torch.concat([msk, y]) # mask + vae_input
        y = y.unsqueeze(0)
...
class WanVideoUnit_FunControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, control_video, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y):
        if control_video is None:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        control_video = pipe.preprocess_video(control_video) # Image 2 tensor
...
# ALL_UNITS = {
#     'ShapeChecker': WanVideoUnit_ShapeChecker,
#     'NoiseInitializer': WanVideoUnit_NoiseInitializer,
#     'InputVideoEmbedder': WanVideoUnit_InputVideoEmbedder,
#     'PromptEmbedder': WanVideoUnit_PromptEmbedder,
#     'ImageEmbedder': WanVideoUnit_ImageEmbedder,
#     'FunControl': WanVideoUnit_FunControl,
#     'FunReference': WanVideoUnit_FunReference,
#     'FunCameraControl': WanVideoUnit_FunCameraControl,
#     'SpeedControl': WanVideoUnit_SpeedControl,
#     'VACE': WanVideoUnit_VACE,
#     'UnifiedSequenceParallel': WanVideoUnit_UnifiedSequenceParallel,
#     'TeaCache': WanVideoUnit_TeaCache,
#     'CfgMerger': WanVideoUnit_CfgMerger,
# }
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    context = dit.text_embedding(context) # mlp -> silu -> mlp

    x = latents # pure noise
    # Merged cfg
...
def vae_normal_output_to_image(vae_output, pattern="B C H W", min_value=-1, max_value=1):
    # Transform a torch.Tensor to PIL.Image
    if pattern != "H W C":
...
def vae_normal_output_to_video(vae_output, pattern="B C T H W", min_value=-1, max_value=1):
    # Transform a torch.Tensor to list of PIL.Image
    if pattern != "T H W C":
        vae_output = reduce(vae_output, f"{pattern} -> T H W C", reduction="mean")
    video_normal_unit = []
    for image in vae_output:
        normal_unit= vae_normal_output_to_image(image, pattern="H W C", min_value=min_value, max_value=max_value)
